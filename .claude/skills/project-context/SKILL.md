...
| Listing card, one-button orchestration, readiness           | `src/features/workspace/ListingSetupCard.tsx`, `src/app/stores/listing-store.ts` (`createListing`), `src/app/listing-readiness.ts` |
...
- **One listing card, one button.** `createListing` runs photos + text with `Promise.allSettled`, skips a part whose provider is unusable and reports both outcomes; the label comes from `listingReadiness` and never promises a part that will not run. One mannequin per device (`Settings.mannequin`), toggled per project (`Project.useMannequin`), never for kids/pets. The seller's brand (`Project.brand`) goes to the copy prompt only — never into an image prompt.
